...
<template>
  <article class="group relative flex flex-col overflow-hidden rounded-2xl border border-line bg-card transition-[box-shadow,translate] duration-200 ease-(--ease-out-strong) hover:-translate-y-0.5 hover:shadow-(--shadow-lift-lg)">
    <div class="relative px-5 pt-5" :style="{ backgroundColor: `color-mix(in srgb, ${bike.color} 5%, #f4f6f2)` }">
      <span v-if="bike.lot" class="roadsign-label absolute left-4 top-4 z-10 rounded-full bg-amber px-2.5 py-1 text-[10.5px] leading-none text-white">
        Lot fin de saison
      </span>
